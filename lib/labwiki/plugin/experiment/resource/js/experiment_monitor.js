define(['omf/data_source_repo', 'theme/labwiki/js/labwiki'], function (data_source_repo, LW) {
  ...
  var experiment_monitor = function(exp_name, el_prefix, properties) {
    ...
    var verify_templ = '<tr id="{id}">\
                        <td class="desc" style="border: 0px;"><span title="{value}" class="{klass}">{name}</span></td>\
                      </tr>';
    ...
    function process_verify_update(name, value) {
      ...
      switch (s.value) {
        case 'true':
          s.klass = 'label label-success'; break;
        case 'false':
          s.klass = 'label label-danger'; break;
        default:
          s_num = Number(s.value)
          if (s_num) {
            if (s_num > 80) {
              s.klass = 'label label-success';
            } else if (s_num > 50) {
              s.klass = 'label label-warning';
            } else {
              s.klass = 'label label-danger';
            }
          } else {
            s.klass = 'label label-default';
          }
          break;
      }
