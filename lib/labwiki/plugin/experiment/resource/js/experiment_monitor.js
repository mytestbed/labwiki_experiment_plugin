  ...
  var experiment_monitor = function(exp_name, el_prefix, properties) {
    ...
    b.save = pc.add_toolbar_button({name: 'stop-experiment', awsome: 'stop', tooltip: 'Stop Experiment', active: true},
      function(ctxt) {
        ...
        LW.execute_controller.request_action(opts, 'POST', function() {
          ctxt.configure({awsome: 'stop-experiment'});
        });
        return false;
      });

    b.dump = pc.add_toolbar_button({name: 'dump', awsome: 'download', tooltip: 'Dump State to iRODS', active: false},
      function(ctxt) {
        ...
        LW.execute_controller.request_action(opts, 'POST', function(reply) {
          ...
          if (reply.success != undefined) {
            $(".alert-dump").show();
            $(".alert-dump").html(reply.success).addClass("alert-success").removeClass("alert-danger");
          } else {
            $(".alert-dump").show();
            $(".alert-dump").html(reply.error).addClass("alert-danger").removeClass("alert-success");
          }
          setTimeout(function() {
            $(".alert-dump").hide();
